...
void logValues(SharedPreferences sharedPreferences) {
  ...
  List<String> keys = sharedPreferences.getKeys().toList().where((String key) => key.startsWith('unitPlan') && key.split('-').length > 2 && key.split('-')[1] == grade).toList();
...
Future convertFromOldVerion(SharedPreferences sharedPreferences) async {
  ...
  for (int i = 0; i < keys.length; i++){
    ...
    int day;
    int unit;
    try { selected = sharedPreferences.getInt(key);
    } catch (e) { return; }
...
int getSelectedIndex(SharedPreferences sharedPreferences, List<UnitPlanSubject> subjects, int day, int unit, {String week = 'A'}) {
  ...
  List<String> keys = subjects.map((UnitPlanSubject subject) => getKey(subject)).toList();
  if (!keys.contains(selected[index])) return null;
  else return keys.indexOf(selected[index]);
}

UnitPlanSubject getSelectedSubject(SharedPreferences sharedPreferences, List<UnitPlanSubject> subjects, int day, int unit, {String week = 'A'}) {
  int index = getSelectedIndex(sharedPreferences, subjects, day, unit, week: week);
...
Future setSelectedSubject(SharedPreferences sharedPreferences, UnitPlanSubject selected, int day, int unit, {UnitPlanSubject selectedB}) async {
  List<String> weeks = selected.week == 'AB' || true ? [getKey(selected)] : [getKey(selected), getKey(selectedB)];
